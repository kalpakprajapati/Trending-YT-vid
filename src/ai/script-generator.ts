...
import { getStoryNarrationPrompt, getTitlePrompt, getDescriptionPrompt } from './prompts.js';
...
export class ScriptGenerator {
   ...
   */
  async generateScript(content: { title: string; body: string; topComments: string[] }): Promise<GeneratedScript> {
    const prompt = getStoryNarrationPrompt(content.title, content.body, content.topComments);
    ...
    return this.withRetry(async () => {
      ...
      // Basic validation
      if (!script.title || !script.scenes || script.scenes.length === 0 || !script.hookLine) {
  ...
  private async withRetry<T>(fn: () => Promise<T>, attempts: number = 3): Promise<T> {
    ...
    for (let i = 0; i < attempts; i++) {
      try {
        ...
        lastError = err;
        console.warn(`[Script Generator] Attempt ${i + 1} failed: ${err instanceof Error ? err.message : String(err)}`);
        if (i < attempts - 1) {
